...
export default function Sidebar({ collapsed, onToggle, onLogout, mobileOpen, onCloseMobile }) {
  ...
  const nav = (
    <aside
      ...
      style={{ width, background: "rgba(10, 15, 23, 0.94)", borderRight: "1px solid var(--border)" }}
    >
      {/* Brand */}
      <div className="flex items-center gap-2.5 px-4" style={{ height: 64, borderBottom: "1px solid var(--border)" }}>
        <span className="flex items-center justify-center w-7 h-7 rounded-lg flex-shrink-0" style={{ background: "var(--primary)", color: "#061009" }}>
          <ShieldCheck size={18} />
        </span>
        {!collapsed && (
    ...
    <>
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <motion.div className="absolute left-0 top-0 h-full" initial={{ x: -240 }} animate={{ x: 0 }} transition={{ duration: 0.2 }}>
            {nav}
          </motion.div>
        </div>
      )}
