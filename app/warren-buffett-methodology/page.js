...
export default function WarrenBuffettMethodology() {
  ...
  function handleAddToWatchlist() {
    ...
    const entry = {
      id: Date.now(),
      companyName: form.companyName || "Unnamed Company",
      score: result.total,
      rating: result.rating,
      ratingColor: result.ratingColor,
      date: new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }),
    };
    const existing = JSON.parse(localStorage.getItem("wm_watchlist") || "[]");
    const alreadyExists = existing.some((e) => e.companyName.toLowerCase() === entry.companyName.toLowerCase());
    if (alreadyExists) { setWatchlistMsg("already"); return; }
    localStorage.setItem("wm_watchlist", JSON.stringify([entry, ...existing]));
