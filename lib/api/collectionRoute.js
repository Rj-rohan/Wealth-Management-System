...
export function createCollectionRoutes(table, allowedFields) {
  ...
  async function POST(request) {
    ...
    await db.insert(table, { advisor_id: user.id, ...pick(body, allowedFields) });
  ...
  async function PUT(request) {
    ...
    const existing = await db.findOne(table, { id: body.id, advisor_id: user.id });
    if (!existing) return fail("Record not found", 404);

    await db.update(table, { id: body.id }, pick(body, allowedFields));
    return ok(await getAggregateProfile(user));
  }
  ...
  async function DELETE(request) {
    ...
    await db.remove(table, { id: body.id, advisor_id: user.id });
    return ok(await getAggregateProfile(user));
  }
