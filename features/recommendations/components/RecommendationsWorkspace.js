...
export default function RecommendationsWorkspace() {
  ...
  const { recs, loading, markActioned } = useRecommendations(clientId);
  ...
  async function handleAction(recId) {
    ...
    success("Recommendation marked as actioned");
  }
    ...
    <div className="space-y-5">
      ...
      {!clientId && (
        <div className="rounded-2xl p-12 text-center" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
          ...
          <p className="text-sm font-medium" style={{ color: "var(--muted-strong)" }}>Select a client to view personalized recommendations</p>
        </div>
      )}
        ...
        <>
          ...
          {filtered.length === 0 && (
            <div className="rounded-2xl p-8 text-center" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
              <p className="text-sm" style={{ color: "var(--muted)" }}>No recommendations in this category</p>
            </div>
          )}
                  ...
                  <Card hover>
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm mt-1.5" style={{ color: "var(--muted-strong)" }}>{r.explanation}</p>
                        <div className="flex flex-wrap items-center gap-4 mt-3 text-xs" style={{ color: "var(--muted)" }}>
                          <span><strong style={{ color: "var(--success)" }}>Benefit:</strong> {r.expectedBenefit}</span>
